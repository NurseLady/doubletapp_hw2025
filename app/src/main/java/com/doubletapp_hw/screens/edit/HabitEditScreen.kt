...
fun HabitEditScreen() {
    ...
    if (loading) {
        ...
        var textCountValue by remember { mutableStateOf(habit.count?.toString() ?: "") }
        ...
        Column(
            ...
            DropdownMenuBox(
                priorityOptions.map { stringResource(it.labelResId) }, habit.priority
            ) { selectedIndex ->
            ...
            Row(modifier = Modifier.padding(vertical = 8.dp)) {
                RadioButtonGroup(
                    ...
                    selectedOption = stringResource(HabitType.entries[habit.type].labelResId)
                ) { selectedIndex ->
                    // Устанавливаем тип привычки на основе выбранного индекса
                    habit = when (selectedIndex) {
                        "Положительная" -> habit.copy(type = HabitType.POSITIVE.ordinal)
                        else -> habit.copy(type = HabitType.NEGATIVE.ordinal)
                    }
                }
            }
            ...
            Row(
                ...
                TextField(
                    ...
                    onValueChange = { input ->
                        ...
                        textCountValue = filteredInput.toIntOrNull()?.toString() ?: ""
                        habit.count = filteredInput.toIntOrNull()
